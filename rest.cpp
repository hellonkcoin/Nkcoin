...
#include <boost/dynamic_nkcset.hpp>
...
// A nkc of a hack - dependency on a function defined in rpc/blockchain.cpp
...
static bool rest_getutxos(HTTPRequest* req, const std::string& strURIPart)
{
    ...
    // check spentness and form a nkcmap (as well as a JSON capable human-readable string representation)
    vector<unsigned char> nkcmap;
    vector<CCoin> outs;
    std::string nkcmapStringRepresentation;
    boost::dynamic_nkcset<unsigned char> hits(vOutPoints.size());
    {
        ...
        for (size_t i = 0; i < vOutPoints.size(); i++) {
            ...
            nkcmapStringRepresentation.append(hits[i] ? "1" : "0"); // form a binary string representation (human-readable for json output)
        }
    }
    boost::to_block_range(hits, std::back_inserter(nkcmap));
    ...
    case RF_BINARY: {
        ...
        ssGetUTXOResponse << chainActive.Height() << chainActive.Tip()->GetBlockHash() << nkcmap << outs;
