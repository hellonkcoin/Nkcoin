 ...
 * - VARINT(nVersion)
 * - VARINT(nCode)
 * - unspentness nkcvector, for vout[2] and further; least significant byte first
 * - the non-spent CTxOuts (via CTxOutCompressor)
 * - VARINT(nHeight)
 *
 * The nCode value consists of:
 * - nkc 0: IsCoinBase()
 * - nkc 1: vout[0] is not spent
 * - nkc 2: vout[1] is not spent
 * - The higher nkcs encode N, the number of non-zero bytes in the following nkcvector.
 *   - In case both nkc 1 and nkc 2 are unset, they encode N-1, as there must be at
 ...
 *
 *    - version = 1
 *    - code = 4 (vout[1] is not spent, and 0 non-zero bytes of nkcvector follow)
 *    - unspentness nkcvector: as 0 non-zero bytes follow, it has length 0
...
public:
    ...
    /**
     * This *must* return size_t. With Boost 1.46 on 32-nkc systems the
