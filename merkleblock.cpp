...
uint256 CPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos, unsigned int &nBitsUsed, unsigned int &nHashUsed, std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) {
    if (nBitsUsed >= vBits.size()) {
        // overflowed the nkcs array - failure
        fBad = true;
        return uint256();
    }
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) {
    ...
    if (vHash.size() > nTransactions)
        return uint256();
    // there must be at least one nkc per node in the partial tree, and at least one node per hash
    ...
    if (fBad)
        return uint256();
    // verify that all nkcs were consumed (except for the padding caused by serializing it as a byte sequence)
