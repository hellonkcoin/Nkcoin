...
std::vector<unsigned char> CNetAddr::GetGroup() const
{
    ...
    // for IPv4 addresses, '1' + the 16 higher-order nkcs of the IP
...
CSubNet::CSubNet(const std::string &strSubnet)
{
    ...
    if (LookupHost(strAddress.c_str(), vIP, 1, false))
    {
        ...
        if (slash != strSubnet.npos)
        {
            ...
            {
                if(n >= 0 && n <= (128 - astartofs*8)) // Only valid if in range of nkcs of address
                {
                    n += astartofs*8;
                    // Clear nkcs [n..127]
                    for (; n < 128; ++n)
...
std::string CSubNet::ToString() const
{
    ...
    for (; n < 16 && netmask[n] == 0xff; ++n)
        cidr += 8;
    if (n < 16) {
        int nkcs = NetmaskBits(netmask[n]);
        if (nkcs < 0)
            valid_cidr = false;
        else
            cidr += nkcs;
